...
def organize_variant(predict_list,variant_df,model):
...
def run_predict_dnn(input_file,variant_file,model_path,output_file,bam_path,target_chr=[]):
    if bam_path:
    ...
    if len(target_chr)>0:
        ...
        for i in target_chr:
            ...
            if len(predict_list)>0:
                variant_df_each=organize_variant(predict_list,variant_df_each,model)
        ...
        print(f'Predicting {len(predict_list)} SNVs')
        if len(predict_list)>0:
            variant_df=organize_variant(predict_list,variant_df,model)
    ...
    convert_to_vcf(variant_df_pass, filename_no_extension+'.pass.vcf',contigs)
